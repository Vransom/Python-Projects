...
def center_window(self,w,h): #pass in the tkinter frame (master) refernece and the w and h
    #get user's screen width and heigh
    screen_width=self.master.winfo_screenwidth()
    screen_height=self.master.winfo_screenheight()
    #calculate x and y coordintes to paint the app centers on the user's screen
...
def addToList(self):
    ...
    var_fname=var_fname.title() #this will ensure the first character in each word is capitalized
    ...
    if (len(var_fname)>0) and (len(var_lname)>0) and len(var_phone)>0 and (len(var_email)>0): #enforce the use to provide all
        conn=sqlite3.connect('phonebook.db')
        with conn:
            cursor=conn.cursor()
            #check the database for existance of the fullname, if so we will alert user and disregard reqest
            ...
            if chkName==0:#if this is 0 then there is no existance of the fullname and we can add new data
                ...
                messagebox.showerror("Name Error","'{}' already exists in the database! Please choose a difference name.".format(var_fullname))
                onClear(self) #call the function to clear all of the textboxes
        conn.commit()
        conn.close()
    else:
...
def onDelete(self):
    ...
    with conn:
        ...
        count.cur.fetchone()[0]
        if count>1:
            confirm=messagebox.askokcancel("Delete Confirmation","All information assosicated with ({}) \nwill be permanantly deleted from the database. \n\nProceed with the deletion request?".format(var_select))
            if confirm:
                conn=sqlite3.connect('phonebook.db')
                with conn:
                    cursor=conn.cursor()
                    cursor.execute("""DELETE FROM tbl_phonebook WHERE col_fullname='{}'""".format(var_select))
                onDeleted(self) #call the function to clear all of the textboxes and selected index of listbox
...
def onRefresh(self):
    #populate the listbox, coinciding with the databse
...
def onUpdate(self):
    try:
        var_select=self.lstList1.curselection()[0] #index of list selecion
